...
local custom_dialog = require "custom.dialog"
...
webview .add_signal( 'init',  function( view )  --  during initialization of a new tab
   ...
   view:add_signal("populate-popup", function (v, menu)
      ...
      print(ins.inspect(v))
      print(ins.inspect(menu))

      table.insert(menu, true)
      table.insert(menu, {"Test Entry", function() v:zoom_in() end })
   end)
   ...
   --  return  false  to prevent requested navigation from taking place
      ...
      elseif low:match(".bibtex$") then
         custom_dialog.dialog('bibtex')
         return false
